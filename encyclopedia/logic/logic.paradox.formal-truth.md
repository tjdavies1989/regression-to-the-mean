...
The parent article diagnosed the liar; this node collects the cures, stated with enough precision to be billed. The task: a theory of a predicate T for which T("A") and A are equivalent, in a language rich enough to build the liar — saying exactly what the theory proves, what it cannot say, and what it costs. Costs are measured, in expressive power and, increasingly, in proof-theoretic strength.
